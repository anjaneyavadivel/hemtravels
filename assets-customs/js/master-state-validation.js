            ...
            form2.validate({
                ...
                submitHandler: function (form)
                {
                    if ($(form).valid())
                    {
                        ...
                        var url = base_url + 'state-master/add';
                        $.ajax({
                            url: url,
                            type: 'post',
                            data: values,
                            success: function (res) {
                                if (res != '') {
                                    ...
                                    window.location.replace(base_url + "state-master");
                                }
            ...
            form30.validate({
                ...
                submitHandler: function (form)
                {
                    if ($(form).valid())
                    {
                        ...
                        values.push({name: "csrf_test_name", value: $.cookie('csrf_cookie_name')});
                        var url = base_url + 'state-master/save-edit';
                        $.ajax({
                            url: url,
                            type: 'post',
                            data: values,

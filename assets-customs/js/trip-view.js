...
jQuery(function ($) {
    ...
    if (available_days != '' && available_days != undefined) {
        ...
        if (available_days.sunday && available_days.sunday != 1) {
            disableDays.push(0)
        }
        if (available_days.monday && available_days.monday != 1) {
            disableDays.push(1)
        }
        if (available_days.tuesday && available_days.tuesday != 1) {
            disableDays.push(2)
        }
        if (available_days.wednesday && available_days.wednesday != 1) {
            disableDays.push(3)
        }
        if (available_days.thursday && available_days.thursday != 1) {
            disableDays.push(4)
        }
        if (available_days.friday && available_days.friday != 1) {
            disableDays.push(5)
        }
        if (available_days.saturday && available_days.saturday != 1) {
            disableDays.push(6)
        }
    ...
    //ENABLE DISABLE DAYS CALENDAR
    var disable_enable_days = $('#disable_enable_days').val();

    if (disable_enable_days != undefined && disable_enable_days != '') {
        disable_enable_days = $.parseJSON(disable_enable_days);
    }
...
//            },
        ...
        beforeShowDay: function (t) { //console.log(t);
            ...
            var valid = ((disable_enable_days != undefined && disable_enable_days != '' && disable_enable_days.length >0 && $.inArray(today, disable_enable_days) < 0)
                        || (disable_enable_days == undefined || disable_enable_days == '' || disable_enable_days.length <= 0)
                    ) &&
                    ($.inArray(t.getDay(), disableDays) >= 0 || $.inArray(today, cutoff_disable_days) >= 0

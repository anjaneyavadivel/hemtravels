 ...
 * @author Anjaneya
 * */
if (!function_exists('tripoffer')) {

    function tripoffer($offerdata = array()) {
        ...
        //ini veriable
        $price_to_adult = 0;$price_to_child = 0; $price_to_infan = 0; $no_of_traveller = 0;
        $no_of_min_booktraveller = 0; $no_of_max_booktraveller = 0;$is_open = 1;
        $parenttrip = getallparenttrip($trip_id);
        print_r($parenttrip);
        $inWhereData = $parenttrip;
        ...
        if ($trip_list->num_rows() > 0) {
            $row = $trip_list->row();
            $pricetoadult = $row->price_to_adult;$pricetochild = $row->price_to_child; $pricetoinfan = $row->price_to_infan;
            $nooftraveller = $row->no_of_traveller;$noofminbooktraveller = $row->no_of_min_booktraveller;
            $noofmaxbooktraveller = $row->no_of_max_booktraveller;
        ...
        if ($trip_list->num_rows() > 0) {
            ...
            $no_of_traveller = $row->no_of_traveller;$no_of_min_booktraveller = $row->no_of_min_booktraveller;
            ...
            $offer_list = selectTable('trip_offers', $whereData, $showField = array('*'), $orWhereData = array(), $group = array(), $order = 'id DESC');
            if ($offer_list->num_rows() > 0) {
                $offer = $offer_list->row();
                if($offer->offer_type==2){
                    $pricetoadult = $offer->price_to_adult;$pricetochild = $offer->price_to_child;
                    ...
                    $price_to_adult = $offer->price_to_adult;$price_to_child = $offer->price_to_child;
                    $price_to_infan = $offer->price_to_infan;
                }
            ...
            if ($CI->session->userdata('user_type') == 'CU') {// super admin coupon changes
                ...
                if ($couponhistory_list->num_rows() > 0 && $ischeckadmin==0) { // coupon for CA from admin
                    ...
                    $pricetoadult = $couponhistory->price_to_adult;$pricetochild = $couponhistory->price_to_child;
            ...
            if (strpos($discount_percentage, '.00') !== false) {
             ...
             $result=array('price_to_adult'=>$price_to_adult,'price_to_child'=>$price_to_child,'price_to_infan'=>$price_to_infan,
                'no_of_traveller'=>$no_of_traveller,'no_of_min_booktraveller'=>$no_of_min_booktraveller,'no_of_max_booktraveller'=>$no_of_max_booktraveller,
                'offer_type'=>$offer_type,'discount_price'=>$discount_price,'coupon_history_id'=>$coupon_history_id,
...
if (!function_exists('trip_book')) {
    ...
    function trip_book($bookdata = array()) {
        ...
        if ($trip_list->num_rows() > 0) {
            ...
            //Offer Specific Day
            $whereData = array('isactive' => 1, 'trip_id'=>$bookdata['trip_id'], 'from_date <='=>$date_of_trip, 'to_date >='=>$date_of_trip);
            $offer_list = selectTable('trip_offers', $whereData, $showField = array('*'), $orWhereData = array(), $group = array(), $order = 'id DESC');
            if ($offer_list->num_rows() > 0) {
                $offer = $offer_list->row();
                if($offer->offer_type==2){

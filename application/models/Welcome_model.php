...
class Welcome_model extends CI_Model {
    ...
    function get_trip_list($where,$limit='') {
        $this->db->select('tm.id,tm.total_booking,tm.price_to_adult,tm.trip_name AS trip_name,tm.trip_img_name AS trip_img_name,tm.created_on AS created_on,tm.isactive AS isactive,tm.status AS status');
    ...
    function get_trippopular_list($where,$limit='') {
        $this->db->select('tm.id,tm.total_booking,tm.trip_name AS trip_name,tm.trip_img_name AS trip_img_name,tm.created_on AS created_on,tm.isactive AS isactive,tm.status AS status');
        $this->db->from('trip_master AS tm');
        $this->db->join('user_master AS um', 'tm.user_id = um.id', 'INNER');
        $this->db->where($where);
